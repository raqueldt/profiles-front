<template>
	<div>
		<div class="row ml-4">
			...
			<div>
				<h3 class="m-2 text-muted">Please wait</h3>
			</div>
		</div>
		<b-row hidden>
			<b-colxx xxs="12">
			...
			<b-colxx xxs="12">
				<b-card class="mb-4">
					...
					<template v-else>
						...
						<template v-else>
							...
							<b-tabs card>
								...
								<b-tab v-if="hasBookings" active>
									<template #title>
										Confirmations<b-badge variant="light"
											class="ml-2">{{ results.bookings.length }}</b-badge>
									</template>
									<b-card-text>
										<!-- filter content -->
										<b-row class="mb-3">
										...
										<!-- data table -->
										<b-table :items="results.bookings" :fields="fields" :filter="filter" small>
                            ...
                            <span v-else class="text-muted">
                                <strong>{{ row.item.cofCodigo }}XX</strong>
											...
											<template #cell(status)="row">
												<span
													:class="resolveStatusColor(row.item.cofEstado)">{{ nameStatusConfirmation(row.item.cofEstado) }}</span>
											</template>
											...
											<template #cell(history)="row">
												...
												<b-button v-b-modal.historyconfirmation variant="primary"
													v-if="Boolean(row.item.cofCodigo)" size="sm"
													@click="historyConfirmation(row.item.cofId)">
													<!--  <i class="fa fa-book"></i> --> Details
												</b-button>

											</template>
											<template #cell(file)="row">
												<b-button variant="primary" v-if="Boolean(row.item.cofCodigo)" size="sm"
													@click="fileManager(row.item)">
													<!--  <i class="fa fa-book"></i> --> File manager
												</b-button>
								...
								<b-tab v-if="hasCotizaciones" active>
									<template #title>
										Quotes<b-badge variant="light"
											class="ml-2">{{ results.cotizaciones.length }}</b-badge>
									</template>
									<b-card-text>
										<!-- filter content -->
										<b-row class="mb-3">
											<b-col lg="4">
												<b-input-group size="sm">
													<b-form-input id="filter-input" v-model="filterCot" type="search"
														placeholder="Type to Search"></b-form-input>
												</b-input-group>
											</b-col>
										</b-row>
										<!-- data table -->
										<b-table :items="results.cotizaciones" :fields="fieldsCot" :filter="filterCot"
											small>
											<template #cell(cotCodigo)="row">
												<router-link :to="{ name: 'quote', params: { gctId: row.item.gctId } }"
													target="_blank"
											...
											<template #cell(status)="row">
												...
												<span
													:style="{ 'color': row.item.colorEstado }">{{ nameStatusQuote(row.item.cotEstado) }}</span>
                      ...
                      -->
												<b-button v-b-modal.historyquote variant="primary"
													v-if="!Boolean(row.item.cofCodigo)" size="sm"
													@click="historyQuote(row.item.cotId)">
													<!--  <i class="fa fa-book"></i> --> Details
												</b-button>
								...
								<b-modal id="historyquote" size="lg" :title="'History Quote'" hide-footer>
									<detail-history-quote :history="history_quote"></detail-history-quote>
								</b-modal>
								<b-modal id="historyconfirmation" size="lg" :title="'History Confirmation'" hide-footer>
									<detail-history-confirmation
										:confirmation="history_confirmation"></detail-history-confirmation>
								</b-modal>


								<b-tab v-if="hasPassengers">
									<template #title>
										Passengers<b-badge variant="light"
											class="ml-2">{{ results.passengers.length }}</b-badge>
									</template>
									<b-card-text>
										<!-- filter content -->
										<b-row class="mb-3">
											<b-col lg="4">
												<b-input-group size="sm">
													<b-form-input id="filter-input" v-model="filterPax" type="search"
														placeholder="Type to Search"></b-form-input>
												</b-input-group>
											</b-col>
										</b-row>
										<!-- data table -->
										<b-table :items="results.passengers" :fields="fieldsPax" :filter="filterPax" small>

											<template #cell(passenger)="row">
												<span><strong>{{ row.item.lpaApellido }}</strong>,
													{{ row.item.lpaNombre }}</span>
											</template>
...
export default {
	...
	methods: {
		...
		fileManager(datos) {
			// this.$router.push({ path: '/collection-file-manager', query: { c: datos.cofClienteId, f: datos.cofId, s: datos.cofFechaVenta, e: datos.cofFechaVenta } })
		},
		handleSearch() {
	...
	async created() {
		...
		await this.search()
		if (this.results.bookings.length)
			this.$router.push({ path: '/collection-file-manager', query: { c: this.results.bookings[0].cofClienteId, f: this.results.bookings[0].cofId, s: this.results.bookings[0].cofFechaVenta, e: this.results.bookings[0].cofFechaVenta } })

		else {
			this.showAlert();
			this.$router.push({ path: '/collection-admin' })
		}
