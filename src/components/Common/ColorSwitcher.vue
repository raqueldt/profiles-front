<template>
  ...
  >
    <div class="p-4">
      <p class="text-muted mb-2">{{ $t("gps.light-theme") }}</p>
      ...
      <p class="text-muted mb-2">{{ $t("gps.dark-theme") }}</p>
      <div class="d-flex flex-row justify-content-between mb-3">
        <a
          href="#"
          v-for="(color,index) in colors.slice(0,5)"
    ...
    <div class="pb-0 pl-4 pt-4">
      <b-form-group label="Border Radius">
        <b-form-radio-group name="radius" v-model="radius">
          <b-form-radio value="rounded" @change="changeRadius('rounded')">Fat</b-form-radio>
          <b-form-radio value="flat" @change="changeRadius('flat')">Rounded</b-form-radio>
        </b-form-radio-group>
      </b-form-group>
    </div>
